...
def Clustering(p,nero,clustering,clustering_rgb):
    print("Clustering...")
    ...
    while (riga < altezza-p):
        ...
        while (colonna < larghezza-p):
            if(nero[riga][colonna] == 255): #definizione
                ...
                max_riga_area = riga+p+1
                max_colonna_area = colonna+p+1
                area = nero[int(riga_area):int(max_riga_area),int(colonna_area):int(max_colonna_area)]
                ...
                if (n_pixel>1):
                    ...
                    pixel = 0
                    riga_area = riga - p
                    while (riga_area<max_riga_area):
                        colonna_area = colonna-p
                        #pixel = 0
                        while (colonna_area<max_colonna_area and pixel<n_pixel):
                    ...
                    #area[0:p*2,0:p*2]=[0]
                    nero[int(riga_area):int(max_riga_area),colonna_area:max_colonna_area]=0
...
for sottocartella in scansione: #ciclo per scansionare le sottocartelle di path
    if sottocartella.is_dir():  #controllo se il file in esame è una cartella
        ...
        for f1 in files:    #Ciclo per scorrere tutte le immagini delle sottocartelle 
            ...
            appoggio=np.zeros((2*p+1, larghezza, 1)).astype(np.uint16)
            appoggio[0:p-1,0:larghezza]=nero[0:p-1,0:larghezza]
            #nero[0:p-1,0:larghezza]=0
            Clustering(int(p/2),appoggio,clustering,clustering_rgb)
            Clustering(p,nero,clustering,clustering_rgb)
