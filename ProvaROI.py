...
# Ogni soggetto è identificato dal QR, che viene decodificato ed utilizzato per rinominare le rispettive sottocartelle. 
        for f1 in files:
            ...
            #upper_range = np.array([50,255,255], dtype=np.uint8)                    #I limiti della maschera
            
            lower_range = np.array([30, 80, 30], dtype=np.uint8)
            upper_range = np.array([150,255,150], dtype=np.uint8) 
